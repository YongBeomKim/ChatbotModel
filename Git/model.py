...
class sequence2sequence:
    ...
    def Make_model(self,output_keep_prob=0.5):
        ...
        with tf.variable_scope('encode'):
            enc_cell=tf.contrib.rnn.MultiRNNCell([self.cell(self.n_hidden,output_keep_prob) for _ in range(self.n_layer)])
            outputs,enc_state=tf.nn.dynamic_rnn(enc_cell,self.enc_input, dtype=tf.float32)
            
        with tf.variable_scope('decode'):
            dec_cell=tf.contrib.rnn.MultiRNNCell([self.cell(self.n_hidden,output_keep_prob) for _ in range(self.n_layer)])
